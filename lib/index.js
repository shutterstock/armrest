...
var Client = function(args) {
	...
	if (args.base) {
		...
		this.basePath = urlComponents.pathname.length > 1 ? urlComponents.pathname : '';
...
['GET', 'POST', 'PUT', 'PATCH', 'HEAD', 'DELETE', 'OPTIONS'].forEach(function(method) {
	...
	Client.prototype[instanceMethod] = function(args) {
		...
		var params = und.cloneDeep(args.params) || {};
		...
		pathname = [this.basePath, pathname].join('');
		...
		this.request(args, function(err, response, body) {
			...
			if (err || responseClass !== 200) {
				...
				err = err || data || getStatusCodeError(response.statusCode);
