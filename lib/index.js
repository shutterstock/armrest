...
['GET', 'POST', 'PUT', 'PATCH', 'HEAD', 'DELETE', 'OPTIONS'].forEach(function(method) {
	...
	Client.prototype[instanceMethod] = function(args) {
		...
		this.request(args, function(err, response, body) {
			...
			var data = body;
			if (response) {
				this.emit('HttpResponse', und.extend({statusCode: response.statusCode, duration: requestDuration, method: requestMethod}, und.pick(originalArgs, ['url'] )));
			} else {
				this.emit('NetworkError', und.extend({statusCode: err.code, duration: requestDuration, method: requestMethod}, und.pick(originalArgs, ['url'] )));
			}
